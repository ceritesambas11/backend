...
// Get attendance by user ID and date range (for payroll calculation)
router.get('/user/:userId', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: records,
      attendances: records // alias for compatibility
    });
  } catch (error) {
...
// ==================== CLOCK IN (FIXED - WITH late_minutes) ====================
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        average_late_minutes: Math.round(avgLate[0].avg_late || 0),
        by_user: byUser
      }
    });
  } catch (error) {
...
router.delete('/:attendanceId', async (req, res) => {
  try {
    ...
    if (record.length > 0) {
      ...
      if (photoOutPath && photoOutPath.startsWith('/uploads/')) {
