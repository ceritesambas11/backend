...
async function sendNotificationToRole(role, title, message, data = {}) {
  try {
    ...
    if (tokens.length === 0) {
      console.log(`⚠️ No FCM tokens found for role: ${role}`);
      return {
        success: false,
    ...
    if (response.failureCount > 0) {
      ...
      if (failedTokens.length > 0) {
        ...
        console.log(`🗑️ Removed ${failedTokens.length} invalid tokens from database`);
      }
    }
...
/**
 * Send push notification to admin role
 * @param {string} title - Notification title
 * @param {string} message - Notification message
 * @param {object} data - Additional data payload
 */
async function sendNotificationToAdmin(title, message, data = {}) {
  return sendNotificationToRole('admin', title, message, data);
}
